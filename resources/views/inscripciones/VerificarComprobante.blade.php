...
<div class="py-4">
    <div class="container">
                    ...
                    <div class="tab-pane fade show active" id="estudiantes" role="tabpanel" aria-labelledby="estudiantes-tab">
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover table-bordered">
                                ...
                                <tbody>
                                    @foreach ($comprobantes as $clave => $grupo)
                                        @php
                                            // Extraer idBoleta y CodigoComprobante de la clave
                                            $partes = explode('-', $clave);
                                            $idBoleta = $partes[0];
                                            $codigoComprobante = $partes[1] ?? '';
                                            
                                            // Obtener el primer elemento del grupo para mostrar los datos
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ basename($primerItem->RutaComprobante) }}</td>
                                            <td>
                                                @if ($primerItem->fecha_actualizacion_verificacion)
                                                    {{ \Carbon\Carbon::parse($primerItem->fecha_actualizacion_verificacion)->format('d/m/Y') }}
                                                @else
                                                    N/A
                                                @endif
                                            </td>
        ...
        data-id="{{ $idBoleta }}"
        data-codigo-comprobante="{{ $codigoComprobante }}"
        data-verificacion-id="{{ $primerItem->verificacion_id }}"
        data-estudiantes="{{ $grupo->pluck('nombre_completo')->unique()->implode(', ') }}"
        data-archivo="{{ basename($primerItem->RutaComprobante) }}"
        data-fecha="{{ $primerItem->fecha_actualizacion_verificacion ? \Carbon\Carbon::parse($primerItem->fecha_actualizacion_verificacion)->format('d/m/Y') : 'N/A' }}"
        data-estado="{{ strtolower($primerItem->status) }}"
        data-nro-comprobante="{{ $codigoComprobante }}"
        data-imagen="{{ route('comprobante.mostrar', [$idBoleta, $codigoComprobante, $primerItem->verificacion_id]) }}">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="row g-3 mb-4">
                    <div class="col-md-4">
                        <div class="mb-3">
                            <label class="form-label text-muted small">ID Boleta:</label>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        let codigoComprobanteActual = null;
        let verificacionIdActual = null;
        ...
        modal.addEventListener('show.bs.modal', function(event) {
            ...
            codigoComprobanteActual = button.getAttribute('data-codigo-comprobante');
            verificacionIdActual = button.getAttribute('data-verificacion-id');
            ...
            console.log('Datos del modal:', {
                id: idBoletaActual,
                codigoComprobante: codigoComprobanteActual,
                verificacionId: verificacionIdActual,
                estudiantes,
                archivo,
                fecha,
                estado,
            ...
            console.log('Estado del comprobante:', estado);
            
            if (estado === 'pendiente') {
                console.log('Mostrando área de decisión');
                areaDecision.classList.remove('d-none');
                areaProcesado.classList.add('d-none');
            } else {
                console.log('Mostrando área procesado');
        ...
        async function loadPDFPreview(pdfSrc, fileName) {
            try {
                ...
                showFileTypeIndicator('PDF', 'Documento PDF');
                showPDFPreview();
                updatePDFControls();
        ...
        function showPreviewError(message = 'No se pudo cargar la vista previa del archivo') {
            ...
            if (previewError) {
                ...
                const errorMessage = previewError.querySelector('p');
                if (errorMessage) {
        ...
        function showFileTypeIndicator(type, description) {
            if (fileTypeIndicator) {
                fileTypeIndicator.textContent = `${type} - ${description}`;
        ...
        modal.addEventListener('hidden.bs.modal', function() {
            resetPreview();
            idBoletaActual = null;
            codigoComprobanteActual = null;
            verificacionIdActual = null;
        ...
        if (aceptarBtn) {
            aceptarBtn.addEventListener('click', function() {
                console.log('Botón aceptar clickeado, ID:', idBoletaActual, 'Código:', codigoComprobanteActual);
                if (!idBoletaActual || !codigoComprobanteActual) return;
                ...
                if (typeof Swal !== 'undefined') {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            aprobarComprobante(idBoletaActual, codigoComprobanteActual);
                        }
                    });
                } else {
        ...
        // Reemplazar el evento de rechazarBtn por esto:
        const rechazarBtn = document.getElementById('rechazar-btn');
        if (rechazarBtn) {
            rechazarBtn.addEventListener('click', function() {
                console.log('Botón rechazar clickeado, ID:', idBoletaActual, 'Código:', codigoComprobanteActual);
                if (!idBoletaActual || !codigoComprobanteActual) return;
                ...
                if (typeof Swal !== 'undefined') {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            // Llamar a la función de rechazo con el motivo (puede ser vacío)
                            rechazarComprobante(idBoletaActual, codigoComprobanteActual, result.value || '');
                        }
                    });
                } else {
                    const motivo = prompt('Motivo del rechazo (opcional):');
                    if (motivo !== null) { // El usuario no canceló
                        rechazarComprobante(idBoletaActual, codigoComprobanteActual, motivo || '');
                    }
                }
            });
        }
        ...
        const confirmarRechazoBtn = document.getElementById('confirmar-rechazo');
        ...
        // Evento para cancelar rechazo
        const cancelarRechazoBtn = document.getElementById('cancelar-rechazo');
        if (cancelarRechazoBtn) {
            cancelarRechazoBtn.addEventListener('click', function() {
                const motivoRechazo = document.getElementById('motivo-rechazo');
                const motivoTextarea = document.getElementById('motivo');
                
                if (motivoRechazo) {
                    motivoRechazo.classList.add('d-none');
                }
                if (motivoTextarea) {
        ...
        function aprobarComprobante(idBoleta, codigoComprobante) {
            const aceptarBtn = document.getElementById('aceptar-btn');
            const rechazarBtn = document.getElementById('rechazar-btn');
            ...
            // Construir URL según el controller
            const url = codigoComprobante ? 
                `/aprobar-comprobante/${idBoleta}/${codigoComprobante}` : 
                `/aprobar-comprobante/${idBoleta}`;
        ...
        // Modificar la función rechazarComprobante para manejar motivo opcional:
        function rechazarComprobante(idBoleta, codigoComprobante, motivo = '') {
            const aceptarBtn = document.getElementById('aceptar-btn');
            const rechazarBtn = document.getElementById('rechazar-btn');
            ...
            const url = codigoComprobante ? 
                `/rechazar-comprobante/${idBoleta}/${codigoComprobante}` : 
                `/rechazar-comprobante/${idBoleta}`;
